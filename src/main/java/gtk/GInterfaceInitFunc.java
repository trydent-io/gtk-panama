...
public interface GInterfaceInitFunc {
    ...
    static MemorySegment allocate(GInterfaceInitFunc fi, SegmentScope scope) {
        return RuntimeHelper.upcallStub(constants$378.GInterfaceInitFunc_UP$MH, fi, constants$378.GInterfaceInitFunc$FUNC, scope);
    }
    static GInterfaceInitFunc ofAddress(MemorySegment addr, SegmentScope scope) {
        MemorySegment symbol = MemorySegment.ofAddress(addr.address(), 0, scope);
        return (java.lang.foreign.MemorySegment _tag, java.lang.foreign.MemorySegment _data) -> {
            try {
                constants$378.GInterfaceInitFunc_DOWN$MH.invokeExact(symbol, _tag, _data);
            } catch (Throwable ex$) {
