...
public interface GBaseInitFunc {
    ...
    static MemorySegment allocate(GBaseInitFunc fi, SegmentScope scope) {
        return RuntimeHelper.upcallStub(constants$376.GBaseInitFunc_UP$MH, fi, constants$376.GBaseInitFunc$FUNC, scope);
    }
    static GBaseInitFunc ofAddress(MemorySegment addr, SegmentScope scope) {
        MemorySegment symbol = MemorySegment.ofAddress(addr.address(), 0, scope);
        return (java.lang.foreign.MemorySegment _user_data) -> {
            try {
                constants$376.GBaseInitFunc_DOWN$MH.invokeExact(symbol, _user_data);
            } catch (Throwable ex$) {
