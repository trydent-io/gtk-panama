...
/**
 * {@snippet :
 * int (*GSocketSourceFunc)(struct _GSocket* socket,enum  condition,void* user_data);
 * }
 */
public interface GSocketSourceFunc {
    ...
    static MemorySegment allocate(GSocketSourceFunc fi, SegmentScope scope) {
        return RuntimeHelper.upcallStub(constants$463.GSocketSourceFunc_UP$MH, fi, constants$463.GSocketSourceFunc$FUNC, scope);
    }
    static GSocketSourceFunc ofAddress(MemorySegment addr, SegmentScope scope) {
        ...
        return (java.lang.foreign.MemorySegment _datagram_based, int _condition, java.lang.foreign.MemorySegment _user_data) -> {
            try {
                return (int)constants$463.GSocketSourceFunc_DOWN$MH.invokeExact(symbol, _datagram_based, _condition, _user_data);
            } catch (Throwable ex$) {
