...
public interface GTestDataFunc {
    ...
    static MemorySegment allocate(GTestDataFunc fi, SegmentScope scope) {
        return RuntimeHelper.upcallStub(constants$305.GTestDataFunc_UP$MH, fi, constants$305.GTestDataFunc$FUNC, scope);
    }
    static GTestDataFunc ofAddress(MemorySegment addr, SegmentScope scope) {
        MemorySegment symbol = MemorySegment.ofAddress(addr.address(), 0, scope);
        return (java.lang.foreign.MemorySegment _user_data) -> {
            try {
                constants$305.GTestDataFunc_DOWN$MH.invokeExact(symbol, _user_data);
            } catch (Throwable ex$) {
