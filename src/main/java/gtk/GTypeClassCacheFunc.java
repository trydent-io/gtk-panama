// Generated by jextract

package gtk;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * int (*GTypeClassCacheFunc)(void* cache_data,struct _GTypeClass* g_class);
 * }
 */
public interface GTypeClassCacheFunc {

    int apply(java.lang.foreign.MemorySegment row, java.lang.foreign.MemorySegment user_data);
    static MemorySegment allocate(GTypeClassCacheFunc fi, SegmentScope scope) {
        return RuntimeHelper.upcallStub(constants$378.GTypeClassCacheFunc_UP$MH, fi, constants$378.GTypeClassCacheFunc$FUNC, scope);
    }
    static GTypeClassCacheFunc ofAddress(MemorySegment addr, SegmentScope scope) {
        MemorySegment symbol = MemorySegment.ofAddress(addr.address(), 0, scope);
        return (java.lang.foreign.MemorySegment _row, java.lang.foreign.MemorySegment _user_data) -> {
            try {
                return (int)constants$378.GTypeClassCacheFunc_DOWN$MH.invokeExact(symbol, _row, _user_data);
            } catch (Throwable ex$) {
                throw new AssertionError("should not reach here", ex$);
            }
        };
    }
}


