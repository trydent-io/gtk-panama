...
public interface GValueTransform {
    ...
    static MemorySegment allocate(GValueTransform fi, SegmentScope scope) {
        return RuntimeHelper.upcallStub(constants$386.GValueTransform_UP$MH, fi, constants$386.GValueTransform$FUNC, scope);
    }
    static GValueTransform ofAddress(MemorySegment addr, SegmentScope scope) {
        MemorySegment symbol = MemorySegment.ofAddress(addr.address(), 0, scope);
        return (java.lang.foreign.MemorySegment _tag, java.lang.foreign.MemorySegment _data) -> {
            try {
                constants$386.GValueTransform_DOWN$MH.invokeExact(symbol, _tag, _data);
            } catch (Throwable ex$) {
