...
public interface GNodeForeachFunc {
    ...
    static MemorySegment allocate(GNodeForeachFunc fi, SegmentScope scope) {
        return RuntimeHelper.upcallStub(constants$130.GNodeForeachFunc_UP$MH, fi, constants$130.GNodeForeachFunc$FUNC, scope);
    }
    static GNodeForeachFunc ofAddress(MemorySegment addr, SegmentScope scope) {
        MemorySegment symbol = MemorySegment.ofAddress(addr.address(), 0, scope);
        return (java.lang.foreign.MemorySegment _tag, java.lang.foreign.MemorySegment _data) -> {
            try {
                constants$130.GNodeForeachFunc_DOWN$MH.invokeExact(symbol, _tag, _data);
            } catch (Throwable ex$) {
