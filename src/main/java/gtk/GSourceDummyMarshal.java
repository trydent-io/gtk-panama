...
public interface GSourceDummyMarshal {
    ...
    static MemorySegment allocate(GSourceDummyMarshal fi, SegmentScope scope) {
        return RuntimeHelper.upcallStub(constants$163.GSourceDummyMarshal_UP$MH, fi, constants$163.GSourceDummyMarshal$FUNC, scope);
    }
    static GSourceDummyMarshal ofAddress(MemorySegment addr, SegmentScope scope) {
        MemorySegment symbol = MemorySegment.ofAddress(addr.address(), 0, scope);
        return () -> {
            try {
                constants$163.GSourceDummyMarshal_DOWN$MH.invokeExact(symbol);
            } catch (Throwable ex$) {
