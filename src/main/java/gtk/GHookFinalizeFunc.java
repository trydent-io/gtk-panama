...
public interface GHookFinalizeFunc {
    ...
    static MemorySegment allocate(GHookFinalizeFunc fi, SegmentScope scope) {
        return RuntimeHelper.upcallStub(constants$151.GHookFinalizeFunc_UP$MH, fi, constants$151.GHookFinalizeFunc$FUNC, scope);
    }
    static GHookFinalizeFunc ofAddress(MemorySegment addr, SegmentScope scope) {
        MemorySegment symbol = MemorySegment.ofAddress(addr.address(), 0, scope);
        return (java.lang.foreign.MemorySegment _tag, java.lang.foreign.MemorySegment _data) -> {
            try {
                constants$151.GHookFinalizeFunc_DOWN$MH.invokeExact(symbol, _tag, _data);
            } catch (Throwable ex$) {
