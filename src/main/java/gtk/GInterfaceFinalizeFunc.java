...
public interface GInterfaceFinalizeFunc {
    ...
    static MemorySegment allocate(GInterfaceFinalizeFunc fi, SegmentScope scope) {
        return RuntimeHelper.upcallStub(constants$378.GInterfaceFinalizeFunc_UP$MH, fi, constants$378.GInterfaceFinalizeFunc$FUNC, scope);
    }
    static GInterfaceFinalizeFunc ofAddress(MemorySegment addr, SegmentScope scope) {
        MemorySegment symbol = MemorySegment.ofAddress(addr.address(), 0, scope);
        return (java.lang.foreign.MemorySegment _tag, java.lang.foreign.MemorySegment _data) -> {
            try {
                constants$378.GInterfaceFinalizeFunc_DOWN$MH.invokeExact(symbol, _tag, _data);
            } catch (Throwable ex$) {
