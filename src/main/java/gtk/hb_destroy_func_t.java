...
public interface hb_destroy_func_t {
    ...
    static MemorySegment allocate(hb_destroy_func_t fi, SegmentScope scope) {
        return RuntimeHelper.upcallStub(constants$878.hb_destroy_func_t_UP$MH, fi, constants$878.hb_destroy_func_t$FUNC, scope);
    }
    static hb_destroy_func_t ofAddress(MemorySegment addr, SegmentScope scope) {
        MemorySegment symbol = MemorySegment.ofAddress(addr.address(), 0, scope);
        return (java.lang.foreign.MemorySegment _user_data) -> {
            try {
                constants$878.hb_destroy_func_t_DOWN$MH.invokeExact(symbol, _user_data);
            } catch (Throwable ex$) {
