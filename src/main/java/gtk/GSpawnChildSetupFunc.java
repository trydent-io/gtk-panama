...
/**
 * {@snippet :
 * void (*GSpawnChildSetupFunc)(void* user_data);
 * }
 */
public interface GSpawnChildSetupFunc {
    ...
    static MemorySegment allocate(GSpawnChildSetupFunc fi, SegmentScope scope) {
        return RuntimeHelper.upcallStub(constants$290.GSpawnChildSetupFunc_UP$MH, fi, constants$290.GSpawnChildSetupFunc$FUNC, scope);
    }
    static GSpawnChildSetupFunc ofAddress(MemorySegment addr, SegmentScope scope) {
        MemorySegment symbol = MemorySegment.ofAddress(addr.address(), 0, scope);
        return (java.lang.foreign.MemorySegment _user_data) -> {
            try {
                constants$290.GSpawnChildSetupFunc_DOWN$MH.invokeExact(symbol, _user_data);
            } catch (Throwable ex$) {
