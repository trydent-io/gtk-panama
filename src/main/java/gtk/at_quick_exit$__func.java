...
public interface at_quick_exit$__func {
    ...
    static MemorySegment allocate(at_quick_exit$__func fi, SegmentScope scope) {
        return RuntimeHelper.upcallStub(constants$51.at_quick_exit$__func_UP$MH, fi, constants$51.at_quick_exit$__func$FUNC, scope);
    }
    static at_quick_exit$__func ofAddress(MemorySegment addr, SegmentScope scope) {
        MemorySegment symbol = MemorySegment.ofAddress(addr.address(), 0, scope);
        return () -> {
            try {
                constants$51.at_quick_exit$__func_DOWN$MH.invokeExact(symbol);
            } catch (Throwable ex$) {
