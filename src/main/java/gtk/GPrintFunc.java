...
public interface GPrintFunc {
    ...
    static MemorySegment allocate(GPrintFunc fi, SegmentScope scope) {
        return RuntimeHelper.upcallStub(constants$246.GPrintFunc_UP$MH, fi, constants$246.GPrintFunc$FUNC, scope);
    }
    static GPrintFunc ofAddress(MemorySegment addr, SegmentScope scope) {
        MemorySegment symbol = MemorySegment.ofAddress(addr.address(), 0, scope);
        return (java.lang.foreign.MemorySegment _user_data) -> {
            try {
                constants$247.GPrintFunc_DOWN$MH.invokeExact(symbol, _user_data);
            } catch (Throwable ex$) {
